    ...
    return function(modules) {
      ...
      function isSameDomain(win) {
        if (!function(win2) {
          ...
          try {
            if (isAboutProtocol(win2) && canReadFromWindow())
              return true;
          } catch (err) {
          }
          try {
            ...
            }(win2) && canReadFromWindow())
              return true;
          } catch (err) {
          }
          try {
            if (getActualDomain(win2) === getActualDomain(window))
              return true;
          } catch (err) {
          }
          return false;
        ...
        try {
          if (win === window)
            return true;
          if (isAboutProtocol(win) && canReadFromWindow())
            return true;
      ...
      function isWindow(obj) {
        ...
        try {
          if ("[object Window]" === {}.toString.call(obj))
            return true;
        } catch (err) {
          if (err && err.message === IE_WIN_ACCESS_ERROR)
            return true;
          ...
          if (!this.dispatching && (resolved || rejected)) {
            ...
            for (var i = 0; i < handlers.length; i++) {
              ...
              if (_result2 instanceof ZalgoPromise && (_result2.resolved || _result2.rejected)) {
                ...
                utils_isPromise(_result2) ? _result2 instanceof ZalgoPromise && (_result2.resolved || _result2.rejected) ? _result2.resolved ? promise.resolve(_result2.value) : promise.reject(_result2.error) : chain(_result2, promise) : promise.resolve(_result2);
            }
            handlers.length = 0;
      ...
      function stringifyError(err, level) {
        ...
        try {
          if (!err)
            return "<unknown error: " + {}.toString.call(err) + ">";
      ...
      memoize(function() {
        ...
        if (uid && "string" == typeof uid)
          return uid;
        if ((uid = script.getAttribute("data-uid-auto")) && "string" == typeof uid)
          return uid;
        if (script.src) {
        ...
        var getName = function() {
          return winPromise.then(function(win) {
            if (!isWindowClosed(win))
              return isSameDomain(win) ? assertSameDomain(win).name : windowNamePromise;
          });
        };
        return {
          id,
          ...
          setLocation: function(href, opts) {
            ...
            return winPromise.then(function(win) {
              ...
              if (0 === href.indexOf("/"))
                href = "" + domain + href;
              else if (!href.match(/^https?:\/\//) && 0 !== href.indexOf(domain))
              ...
              if ("post" === method)
                return getName().then(function(name) {
                  ...
                  !function(_ref3) {
                    ...
                    if (body2)
                      for (var _i24 = 0, _Object$keys4 = Object.keys(body2); _i24 < _Object$keys4.length; _i24++) {
                        var _body$key;
      ...
      function function_serializeFunction(destination, domain, val, key, _ref3) {
        on = (_ref = {
          ...
          return on("postrobot_method", {
            ...
            if (!meth)
              throw new Error("Could not find method '" + name2 + "' with id: " + data.id + " in " + getDomain(window));
            var methodSource = meth.source, domain2 = meth.domain, val2 = meth.val;
            return promise_ZalgoPromise.try(function() {
              if (!matchDomain(domain2, origin))
            ...
            var getDeserializedFunction = function(opts) {
              ...
              function crossDomainFunctionWrapper2() {
                ...
                return window_ProxyWindow.toProxyWindow(source2, {
                  ...
                  if (meth && meth.val !== crossDomainFunctionWrapper2)
      ...
      function on_on(name, options, handler) {
        ...
        var requestListener = function addRequestListener(_ref4, listener) {
          ...
          if (existingListener)
            throw win && domain ? new Error("Request listener already exists for " + name2 + " on domain " + domain.toString() + " for " + (win === getWildcard() ? "wildcard" : "specified") + " window") : win ? new Error("Request listener already exists for " + name2 + " for " + (win === getWildcard() ? "wildcard" : "specified") + " window") : domain ? new Error("Request listener already exists for " + name2 + " on domain " + domain.toString()) : new Error("Request listener already exists for " + name2);
        ...
        return window_ProxyWindow.toProxyWindow(winOrProxyWin, {
          ...
          return promise_ZalgoPromise.try(function() {
            !function(name2, win2, domain) {
              if (!name2)
                throw new Error("Expected name");
              if (domain && "string" != typeof domain && !Array.isArray(domain) && !util_isRegex(domain))
                throw new TypeError("Can not send " + name2 + ". Expected domain " + JSON.stringify(domain) + " to be a string, array, or regex");
              if (isWindowClosed(win2))
                throw new Error("Can not send " + name2 + ". Target window is closed");
            ...
            if (function(parent, child) {
              ...
              if (function(win2) {
                ...
                for (var _i7 = 0, _getAllChildFrames4 = function getAllChildFrames(win3) {
                  ...
                  for (var _i3 = 0, _getFrames2 = getFrames(win3); _i3 < _getFrames2.length; _i3++) {
                    ...
                    result.push(frame2);
                    for (var _i5 = 0, _getAllChildFrames2 = getAllChildFrames(frame2); _i5 < _getAllChildFrames2.length; _i5++)
              ...
              return function(win2, timeout, name2) {
      ...
      function cleanUpWindow(win) {
        for (var _i2 = 0, _requestPromises$get2 = windowStore("requestPromises").get(win, []); _i2 < _requestPromises$get2.length; _i2++)
          _requestPromises$get2[_i2].reject(new Error("Window " + (isWindowClosed(win) ? "closed" : "cleaned up") + " before response")).catch(src_util_noop);
      }
      setup();
    }]);
  });
...
//# sourceMappingURL=index-DD_kndDs-OKWK3TMI.js.map
